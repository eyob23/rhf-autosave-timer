...
function HouseholdFormSection({ applicationId }: { applicationId: string }) {
  const { controller, form, ready, loadError } =
    useAutosaveSection<HouseholdForm>({
      ...
      load: (signal) =>
        runApiRequest(
          store.dispatch(
            applicationApi.endpoints.getHousehold.initiate(applicationId),
          ),
          signal,
        ),
      save: (values, signal) =>
        runApiRequest(
          store.dispatch(
            applicationApi.endpoints.updateHousehold.initiate({
              applicationId,
              values,
            }),
          ),
  ...
  if (loadError)
    return <p role="alert">Could not load household information.</p>;
  ...
  return (
    <section className="section-panel">
      ...
      <header className="section-header">
        <div>
          <p className="eyebrow">Section 3 of 4</p>
          <h2>Household</h2>
          <p>Living arrangements and everyone included in this application.</p>
        </div>
              ...
              <div className="array-row" key={member.fieldId}>
                ...
                <div className="form-grid form-grid--three">
                  ...
                  <label>
                    ...
                    <input
                      type="date"
                      {...form.register(`members.${index}.dateOfBirth`)}
                    />
                  </label>
                  <label className="choice choice--standalone">
                    <input
                      type="checkbox"
                      {...form.register(`members.${index}.dependent`)}
                    />{" "}
                    Financial dependent
                  </label>
                </div>
